...
class Sistema {
  private:
		...
		int Id; //Contador para a quantidade de usuários cadastrados no sistema
		

  public:
		/*!Construtor de Sistema para inicializar o contador de usuários (int cont)
		e o indicador de usuário logado(int usuarioLogado)
		*/
		Sistema();

		/*! Get para o atributo Id
			@return um inteiro com o último Id
		*/
		int getId();

		/*! Checa se o usuário existe (foi registrado no sistema))
			@param id o id do usuário que será checado
			@return true usuário existe
			@return false usuário não existe
		*/
		bool checaUsuarioExiste(int id);
		...
		*/
		...
		/*! Compara se o email de entrada é igual ao email de um objeto usuário
				@param email o email de entrada do usuário.
				@return true se o email é igual
				@return false se o email é diferente.
		*/
		static auto checaEmailJaCadastradoPredicate ( const string &email ){
			return [&](Usuario &user){return user.getEmail() == email;};
		}
		
		/*! Compara se o email e senha de entrada são iguais ao de um determinado objeto usuário para efetuar o login.
				@param email de entrada do usuário.
				@param senha de entrada do usuário.
				@return true se são iguais.
				@return false se são diferentes.
		*/
		static auto checaEmailESenhaValidosPredicate ( const string &email, const string &senha ){
			return [&](Usuario &user){return user.getEmail() == email and user.getSenha() == senha;};
		}
		
		/*! Compara se o nome de um servidor é igual ao nome de um servidor de um objeto servidor.
				@param nome do servidor
				@return true se são iguais.
				@return false se são diferentes.
		*/
		static auto checaServidorExistePredicate (const string &nome){
			return [&](Servidor &server){return server.getNomeServidor() == nome;};
		}
};
